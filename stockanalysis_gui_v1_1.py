...
# Thread-local session — each worker thread reuses its own persistent HTTP connection
...
class App(tk.Tk):
    ...
    def _run_scrape(self, tickers, periods, output_dir, fmt, statements, mode, timestamp):
        ...
        # Fetch all pages concurrently — 8 workers handles typical loads well
        results_map = {}
        with ThreadPoolExecutor(max_workers=8) as pool:
            future_to_job = {
                pool.submit(scrape_table, t, s, p): (t, s, p)
                for t, s, p in jobs
            }
            for future in as_completed(future_to_job):
                ...
                try:
                    ...
                    results_map[job] = ScrapeError(str(e))
                done += 1
                self.progress["value"] = (done / total) * 50  # first 50% = fetching
        ...
        for ticker in tickers:
            ...
            for statement in statements:
                ...
                for period in periods:
                    if statement == "overview" and period == "quarterly":
                        continue
                    result = results_map.get((ticker, statement, period))
                    ...
                    self.progress["value"] = 50 + (done / total) * 50  # second 50% = saving
